...
local distance = decode_params["name"]
local city = decode_params["city"]
...
--直接进行数据库查询
local res=iquery.query(string.format([[insert into park_lots (name,city,lat,lon) values('%s','%s',%s,%s) returning lots_id]],name,city,lat,lon))
