...
static int encrypt(lua_State* pState)
{
    ...
    int i, j, len;
	//gu.qinghuan@guodongiot.com pkcs7padding aec128 start
	unsigned char *paddedStr = NULL;
	int paddingLength = 0;
	//end
	...
	//gu.qinghuan@guodongiot.com pkcs7padding aec128 start
	paddingLength = BLOCK_SIZE - len % BLOCK_SIZE;
	if(paddingLength != 0){
		paddedStr = (unsigned char *)malloc(len + paddingLength);
		if (!paddedStr)
	    {
	        luaL_error(pState, "alloc padding memory error");
	        return 0;
	    }
		
		memcpy(paddedStr,str,len);
		memset(paddedStr + len,paddingLength,paddingLength);
		str = paddedStr;
		len += paddingLength;
	}
	//end
...
static int decrypt(lua_State* pState)
{
	...
	//gu.qinghuan@guodongiot.com pkcs7padding aec128 start
    //lua_pushlstring(pState, buffer, len);
	lua_pushlstring(pState, buffer, len - buffer[len -1]);
	//end
    free(buffer);
    return 1;
}
